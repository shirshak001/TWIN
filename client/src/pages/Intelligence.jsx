...
function Intelligence() {
  return (
    <div className="min-h-full bg-(--secondary-bg) px-6 py-6 text-(--text) sm:px-8">
      <header className="mb-6">
        <p className="text-sm font-semibold uppercase tracking-[0.24em] text-(--muted)">Digital Brain</p>
        <h1 className="mt-3 text-4xl font-bold text-(--text)">AI Intelligence</h1>
      ...
      <section className="mb-6 grid gap-4 xl:grid-cols-4">
        {intelligenceCards.map((card) => (
          <article key={card.title} className="rounded-3xl border border-(--border) bg-(--surface) p-5 shadow-lg transition hover:shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-(--muted)">{card.status}</p>
                <h2 className="mt-2 text-xl font-bold text-(--text)">{card.title}</h2>
              </div>
              <card.icon className="h-6 w-6 text-(--muted)" />
            </div>
      ...
      <section className="mb-6 grid gap-4 xl:grid-cols-12">
        <article className="xl:col-span-7 rounded-3xl border border-(--border) bg-(--surface) p-6 shadow-lg transition hover:shadow-xl">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-(--muted)">Adaptive AI</p>
              <h3 className="mt-2 text-2xl font-bold text-(--text)">Behavioral adaptation engine</h3>
            </div>
        ...
        <article className="xl:col-span-5 rounded-3xl border border-(--border) bg-(--surface) p-6 shadow-lg transition hover:shadow-xl">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-(--muted)">Explainable AI</p>
              <h3 className="mt-2 text-2xl font-bold text-(--text)">Why this recommendation?</h3>
            </div>
      ...
      <section className="grid gap-4 xl:grid-cols-12">
        <article className="xl:col-span-8 rounded-3xl border border-(--border) bg-(--surface) p-6 shadow-lg transition hover:shadow-xl">
          <h3 className="text-lg font-semibold text-(--text)">Cross-Domain Intelligence</h3>
          <p className="mt-2 text-sm leading-6 text-(--muted)">Correlations between health, finance, and career give you a complete system view.</p>
        ...
        <article className="xl:col-span-4 rounded-3xl border border-(--border) bg-(--surface) p-6 shadow-lg transition hover:shadow-xl">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-(--text)">AI Confidence</h3>
      ...
      <section className="mt-6 rounded-3xl border border-(--border) bg-(--surface) p-6 shadow-lg transition hover:shadow-xl">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-(--muted)">Future Predictions</p>
            <h3 className="mt-2 text-2xl font-bold text-(--text)">Scenario impact overview</h3>
          </div>
          <ArrowRight className="h-5 w-5 text-(--muted)" />
        </div>
        <div className="mt-6 overflow-hidden rounded-3xl border border-(--border) bg-(--surface-soft) p-4">
...
function ConfidenceBadge({ label, value }) {
  return (
    <div className="rounded-3xl border border-(--border) bg-(--surface-soft) px-4 py-3 text-sm font-semibold text-(--text)">
      <p>{label}</p>
...
function CrossIntelligenceItem({ title, text, badge, color }) {
  ...
  return (
    <div className="rounded-3xl border border-(--border) bg-(--surface) p-4 shadow-lg">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-semibold text-(--text)">{title}</p>
        <span className="rounded-full bg-[rgba(255,255,255,0.08)] px-3 py-1 text-xs font-semibold" style={{ color: colorMap[color] }}>{badge}</span>
      </div>
