  ...
  return (
    <div className="min-h-full bg-(--secondary-bg) px-6 py-6 text-(--text) sm:px-8">
      <header className="mb-6">
        <p className="text-sm font-semibold uppercase tracking-[0.24em] text-(--muted)">What-if Engine</p>
        <h1 className="mt-3 text-4xl font-bold text-(--text)">AI Simulation</h1>
      ...
      <section className="mb-6 grid gap-4 xl:grid-cols-12">
          ...
          {scenarios.map((scenario) => (
            <button
              ...
              className={`w-full rounded-3xl border p-5 text-left transition-all duration-300 ${
                activeScenario === scenario.label ? 'border-(--primary) bg-[rgba(124,255,178,0.16)] shadow-sm' : 'border-(--border) bg-(--surface) hover:border-[rgba(255,255,255,0.12)] hover:shadow-sm'
              }`}>
              <div className="mb-3 flex items-center justify-between gap-3 text-(--text)">
        ...
        <article className="xl:col-span-9 rounded-3xl border border-(--border) bg-(--surface) p-6 shadow-lg transition hover:shadow-xl">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.16em] text-(--muted)">Selected scenario</p>
              <h2 className="mt-2 text-2xl font-bold text-(--text)">{selected.label}</h2>
            </div>
          ...
          <div className="mt-8 rounded-3xl border border-(--border) bg-(--surface-soft) p-5">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.14em] text-(--muted)">Future Trajectory</p>
                <h3 className="mt-2 text-lg font-bold text-(--text)">Projected outcome graph</h3>
              </div>
              <Shield className="h-5 w-5 text-(--muted)" />
            </div>
            <div className="relative h-56 overflow-hidden rounded-3xl border border-(--border) bg-(--surface) p-4">
              ...
              <div className="absolute inset-x-0 bottom-4 px-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-(--muted) flex justify-between">
                <span>Today</span>
                <span>1 mo</span>
                <span>3 mo</span>
                <span>6 mo</span>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <article className="rounded-3xl border border-(--border) bg-(--surface) p-5 shadow-lg">
              <p className="text-sm font-semibold text-(--text)">Impact Summary</p>
              <p className="mt-3 text-sm leading-6 text-(--muted)">{selected.impact}</p>
            </article>
            <article className="rounded-3xl border border-(--border) bg-(--surface) p-5 shadow-lg">
              <p className="text-sm font-semibold text-(--text)">Recommendations</p>
...
function ImpactCard({ label, value, color }) {
  ...
  return (
    <div className="rounded-3xl border border-(--border) bg-(--surface) p-5 shadow-lg transition hover:shadow-xl">
